<template>
  <div class="modal fade" id="userConfigModal" ref="modalRef" tabindex="-1" aria-hidden="true">
          ...
          <div class="modal-content rounded-3xl border-0 shadow-2xl font-sans overflow-hidden">
              ...
              <div class="modal-body bg-slate-50/50 p-6 space-y-4">
                  ...
                  <div>
                      ...
                      <input type="tel" v-model="userConfigForm.Phone" @input="onPhoneInput" class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none font-bold text-sm text-slate-800 shadow-sm focus:border-indigo-500">
                  </div>
              </div>
              <div class="modal-footer border-t border-slate-100 px-6 py-5 bg-white flex gap-3">
                  <button type="button" class="flex-1 px-4 py-3 border border-slate-205 text-slate-600 rounded-xl bg-white hover:bg-slate-50 font-bold text-sm transition cursor-pointer" data-bs-dismiss="modal">Отмена</button>
                  <button type="button" class="flex-1 px-5 py-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 font-bold text-sm transition shadow-lg shadow-indigo-200 flex items-center justify-center gap-2 border-none cursor-pointer" @click="saveUserConfig" :disabled="isSavingUserConfig">
                      <span v-if="isSavingUserConfig" class="spinner-border spinner-border-sm text-white border-2"></span>Сохранить
                  </button>
              </div>
          </div>
      </div>
  ...
  methods: {
    ...
    async saveUserConfig() {
      ...
      try {
          ...
          this.store.dispatchSync('approveUser', { id: obj.ID, data: { Role: obj.Role, Name: obj.Name, Phone: obj.Phone } }, 'Users');
