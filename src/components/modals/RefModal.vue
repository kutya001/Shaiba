<template>
  <div class="modal fade" id="refModal" ref="modalRef" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered modal-sm">
          <div class="modal-content rounded-3xl border-0 shadow-2xl font-sans overflow-hidden">
              <div class="modal-header border-b border-slate-100 px-6 py-5 bg-white">
                  <h5 class="modal-title font-bold text-slate-800 m-0">{{ refForm.ID ? 'Редактировать:' : 'Добавить:' }} {{ refMeta[activeRefTab] ? refMeta[activeRefTab].title : '' }}</h5>
              ...
              <div class="modal-body bg-slate-50/50 p-6 space-y-4" v-if="refMeta[activeRefTab]">
              ...
              <div class="modal-footer border-t border-slate-100 px-6 py-5 bg-white flex gap-3">
                  ...
                  <button type="button" class="flex-1 px-5 py-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 font-bold text-sm transition shadow-lg shadow-indigo-200 flex items-center justify-center gap-2 border-none cursor-pointer" @click="saveRef" :disabled="isSaving">
    ...
    async saveRef() {
      try {
          ...
          if (isNew) {
              ...
              if(!this.store.db[this.activeRefTab]) this.store.db[this.activeRefTab] = [];
              this.store.db[this.activeRefTab].push(payload);
              this.store.dispatchSync('addRow', payload, sheet);
          } else {
              let idx = this.store.db[this.activeRefTab].findIndex(x => x.ID === payload.ID);
              if(idx > -1) this.store.db[this.activeRefTab][idx] = payload;
              this.store.dispatchSync('updateRow', payload, sheet);
          }
          
          this.store.showToast(payload.ID && !isNew ? 'Обновлено в справочнике' : 'Добавлено в справочник');
          this.hide();
      } catch(e) {
