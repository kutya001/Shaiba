<template>
  <div class="modal fade" id="profileModal" ref="modalRef" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered modal-md">
          <div class="modal-content rounded-3xl border-0 shadow-2xl font-sans overflow-hidden">
              <div class="modal-header border-b border-slate-100 px-6 py-5 bg-white">
                  ...
                  <button type="button" class="btn-close text-slate-400 focus:ring-0 shrink-0 border-none bg-transparent" data-bs-dismiss="modal"></button>
              </div>
              <div class="modal-body bg-slate-50 p-6 space-y-4">
                  <!-- View mode -->
                  <div v-if="!isEditingProfile" class="flex flex-col items-center justify-center p-6 bg-white rounded-2xl border border-slate-100 shadow-sm relative animate-fade-in">
                      <button @click="isEditingProfile = true" class="absolute top-4 right-4 p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition border-none bg-transparent" title="Редактировать">
                      ...
                      <div class="w-20 h-20 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center text-3xl font-bold mb-4 shadow-inner">
                          {{ user && user.Username ? user.Username.slice(0,2).toUpperCase() : '?' }}
                      </div>
                      <h3 class="font-bold text-xl text-slate-800 mb-1" v-if="user">{{ user.Name || user.Username }}</h3>
                      <p class="text-sm font-semibold text-slate-500 mb-4" v-if="user">@{{ user.Username }}</p>
                      
                      <div class="flex items-center gap-2 px-3 py-1.5 bg-slate-100 rounded-lg mb-2" v-if="user">
                      ...
                      <div class="flex items-center gap-2 px-3 py-1.5 rounded-lg text-slate-700" v-if="user && user.Phone">
                      ...
                      <button @click="triggerLogout" class="flex items-center justify-center gap-2 w-full py-2.5 text-red-600 hover:bg-red-50 rounded-xl font-bold transition border-none bg-transparent cursor-pointer">
                          <i class="bi bi-box-arrow-right"></i> Выйти из аккаунта
                      </button>
                  </div>
                  
                  <!-- Edit mode -->
                  <div v-else class="space-y-4 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm relative animate-fade-in">
                      <button @click="isEditingProfile = false" class="absolute top-3 left-3 p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition border-none bg-transparent" title="Отмена">
                      ...
                      <div>
                          ...
                          <input v-model="profileForm.Phone" type="tel" @input="onPhoneInput" class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl font-bold text-sm text-slate-800 outline-none focus:border-indigo-500 shadow-sm transition" placeholder="+996 XXX XXX XXX">
                      </div>
                      <div>
                      ...
                      <button @click="saveUserProfile" :disabled="isSavingProfile" class="w-full py-3.5 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition flex items-center justify-center gap-2 shadow-md mt-6 border-none cursor-pointer">
                          <span v-if="isSavingProfile" class="spinner-border spinner-border-sm text-white"></span><span>Сохранить настройки</span>
                      </button>
                  </div>
              </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    async saveUserProfile() {
      if (!this.profileForm.username) return this.store.showToast('Укажите имя пользователя', 'error');
